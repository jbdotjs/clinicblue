...
export default function HomeAnimated() {
  ...
  return (
    <div className="overflow-x-hidden">
      {/* Hero */}
      <section ref={heroRef} className="relative max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 py-16 md:py-24">
          ...
          >
            <div className="flex flex-col gap-4">
              <motion.span
                ...
                className="inline-flex items-center gap-2 text-primary/70 text-sm font-semibold tracking-wider uppercase"
              >
            ...
            >
              <Link
                href="/services"
                className="flex items-center gap-2 rounded-full h-14 px-8 bg-primary text-white text-base font-bold tracking-wide shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-soft-hover)] hover:-translate-y-1 transition-all duration-300"
              >
                Discover Treatments
              ...
              <Link
                href="/journey"
                className="flex items-center gap-2 text-slate-700 font-medium hover:text-primary transition-colors group"
              >
                <span className="w-10 h-10 rounded-full border-2 border-slate-200 flex items-center justify-center group-hover:border-primary group-hover:bg-primary/5 transition-colors">
      ...
      <section className="relative py-16 md:py-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 flex flex-col gap-12">
          ...
          >
            ...
            <h2 className="text-slate-900 text-4xl md:text-5xl font-bold tracking-tight leading-tight">
              Our Services
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div
                key={service.title}
                className="group relative flex flex-col gap-6 rounded-[2rem] bg-surface p-8 shadow-[var(--shadow-soft)] border border-slate-100/50 overflow-hidden cursor-default"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-bl-[100px] transition-transform duration-500 group-hover:scale-110 -z-10" />
                <motion.div
                  className="text-primary bg-gradient-to-br from-primary/10 to-accent/10 w-16 h-16 rounded-2xl flex items-center justify-center"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 py-16 md:py-24">
        <motion.div
          className="relative rounded-[2.5rem] bg-gradient-to-br from-primary/8 to-accent/5 p-10 md:p-16 flex flex-col items-center text-center overflow-hidden"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, ease: "easeOut" }}
        >
          <div className="absolute top-6 left-6 md:top-8 md:left-8 text-primary/10">
            <span
              className="material-symbols-outlined text-8xl md:text-9xl"
          ...
          <div className="max-w-3xl relative z-10">
            <p className="text-2xl md:text-3xl font-medium text-slate-800 leading-relaxed italic mb-8">
              &ldquo;The moment I stepped in, my anxiety melted away. The hydrotherapy
              session was truly a restorative experience that left me feeling
              completely renewed.&rdquo;
            </p>
